...
class CountriesView extends Component {
  ...
  renderVisitedCountries = () => {
    ...
    const updatedVisitedCountries = countriesList.filter(
      eachItem => eachItem.isVisited === true,
    )
    console.log(updatedVisitedCountries)
    this.setState({visitedCountriesList: updatedVisitedCountries})
  }
  ...
  onRemove = id => {
    ...
    this.setState(
      {countriesList: updatedCountryList},
    ...
    return (
      <div className="main-bg">
        <div className="content-container">
          <h1 className="title-element">Countries</h1>
          <ul className="countries-unordered-list-styling">
            {countriesList.map(eachItem => (
              <li className="list-item-styling" key={eachItem.id}>
                <p className="title-element">{eachItem.name}</p>
                {eachItem.isVisited ? (
                  ...
                  <button
                    ...
                    onClick={() => this.onClickVisit(eachItem.id)}
                  >
          ...
          <h1 className="title-element">Visited Countries</h1>
          <ul className="visited-countries-unordered-list">
          ...
          {visitedCountriesList.length === 0 && (
            <p className="no-countries-text-element">
